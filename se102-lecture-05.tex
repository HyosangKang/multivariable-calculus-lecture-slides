...
\[ \iint_D fdxdy = \iint_{[a,b]\times[c,d]}Fdxdy.\]
\end{defn}
\begin{thm}
...
on a boxed region 
	\[[x_0^0, x_1^0]\times [x_0^1,x_1^1]\times \cdots\times[x_0^{n-1}, x_1^{n-1}]\]
...
\end{rem}
\begin{thm}
If $f(x,y,z)$ is defined on a bounded region $V\subset\mathbf R^3$ and
$V$ is contained in a boxed region $[a,b]\times[c,d]\times[e,f]$,
then $f$ is integrable if $F$ below is integrable on $[a,b]\times[c,d]\times[e,f]$.
